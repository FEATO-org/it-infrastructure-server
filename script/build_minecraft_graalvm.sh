...
proxy_image="${MINECRAFT_PROXY_IMAGE:-it-infrastructure/mc-proxy:java25-graalvm}"
...
docker build --pull --no-cache --build-arg BASE_IMAGE=itzg/mc-proxy:java25 \
  --tag "${proxy_image}" --file "${REPO_ROOT}/minecraft/Dockerfile.graalvm" "${REPO_ROOT}/minecraft"
